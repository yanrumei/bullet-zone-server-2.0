...
/*      */         {
/*  651 */           this.frame[(frameIndex++)] = (0x1700000 | this.cw.addType((String)local[i]));
/*  652 */         } else if ((local[i] instanceof Integer)) {
/*  653 */           this.frame[(frameIndex++)] = ((Integer)local[i]).intValue();
/*      */         }
/*      */         else {
/*  656 */           this.frame[(frameIndex++)] = (0x1800000 | this.cw.addUninitializedType("", ((Label)local[i]).position));
/*      */         }
/*      */       }
/*      */       
...
/*      */         {
/*  663 */           this.frame[(frameIndex++)] = (0x1700000 | this.cw.addType((String)stack[i]));
/*  664 */         } else if ((stack[i] instanceof Integer)) {
/*  665 */           this.frame[(frameIndex++)] = ((Integer)stack[i]).intValue();
/*      */         }
/*      */         else {
/*  668 */           this.frame[(frameIndex++)] = (0x1800000 | this.cw.addUninitializedType("", ((Label)stack[i]).position));
/*      */         }
/*      */       }
/*      */       
/*  672 */       endFrame();
...
/*      */         }
/*      */         else {
/*  724 */           this.stackMap.putByte(247).putShort(delta);
/*      */         }
...
/*      */     }
/*      */     
/*  828 */     if ((var < 4) && (opcode != 169)) { int opt;
/*      */       int opt;
/*  830 */       if (opcode < 54)
/*      */       {
...
/*  883 */         int size; int size; int size; int size; switch (opcode) {
/*      */         case 178: 
...
/*      */     {
/* 1228 */       this.code.putByte(196).put12(132, var).putShort(increment);
/*      */     } else {
...
/*      */     {
/*      */ 
/* 1423 */       bv.putShort(start[i].position).putShort(end[i].position - start[i].position).putShort(index[i]);
/*      */     }
...
/* 1461 */         Label l = handler.start.getFirst();
...
/*      */       {
/* 1501 */         Label l = changed;
/* 1502 */         changed = changed.next;
/* 1503 */         l.next = null;
/* 1504 */         f = l.frame;
/*      */         
/* 1506 */         if ((l.status & 0x10) != 0) {
/* 1507 */           l.status |= 0x20;
/*      */         }
/*      */         
/* 1510 */         l.status |= 0x40;
/*      */         
/* 1512 */         int blockMax = f.inputStack.length + l.outputStackMax;
/* 1513 */         if (blockMax > max) {
/* 1514 */           max = blockMax;
/*      */         }
/*      */         
/* 1517 */         Edge e = l.successors;
/* 1518 */         while (e != null) {
...
/* 1553 */             this.frame[frameIndex] = 
/* 1554 */               (0x1700000 | this.cw.addType("java/lang/Throwable"));
/* 1555 */             endFrame();
/*      */             
/*      */ 
...
/*      */             {
/* 1620 */               id++;
/* 1621 */               subroutine.visitSubroutine(null, id / 32L << 32 | 1L << id % 32, this.subroutines);
/*      */             }
/*      */           }
/*      */           
/* 1625 */           l = l.successor;
...
/*      */     
/*      */ 
/* 1760 */     for (int i = 0; i < locals.length; i++) {
/* 1761 */       int t = locals[i];
/* 1762 */       if (t == 16777216) {
/* 1763 */         nTop++;
/*      */       } else {
...
/*      */         }
/*      */         
/* 1844 */         this.frame[(frameIndex++)] = (0x1700000 | this.cw.addType(this.descriptor.substring(j, ++i)));
/* 1845 */         break;
/*      */       case 'L': 
...
/*      */         }
/*      */         
/* 1851 */         this.frame[(frameIndex++)] = (0x1700000 | this.cw.addType(this.descriptor.substring(j + 1, i++)));
/* 1852 */         break;
...
/*      */   {
/* 1904 */     int clocalsSize = this.frame[1];
...
/* 1915 */     int k = 0;
/*      */     int delta;
/* 1917 */     int delta; if (this.frameCount == 0) {
...
/* 2275 */         while (h != null)
/*      */         {
/* 2277 */           out.putShort(h.start.position).putShort(h.end.position).putShort(h.handler.position).putShort(h.type);
/* 2278 */           h = h.next;
/*      */         }
/*      */       }
...
/*      */     {
/* 2356 */       out.putShort(this.cw.newUTF8("Signature")).putInt(2).putShort(this.cw.newUTF8(this.signature));
/*      */     }
...
/* Location:              C:\Users\ikatwal\Downloads\bullet-zone-server-2.0.jar!\BOOT-INF\lib\spring-core-4.3.14.RELEASE.jar!\org\springframework\asm\MethodWriter.class
