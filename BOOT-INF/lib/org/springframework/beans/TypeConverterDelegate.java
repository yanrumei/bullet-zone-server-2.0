...
/*     */   
/*  63 */   static { try { Class<?> clazz = ClassUtils.forName("java.util.Optional", TypeConverterDelegate.class.getClassLoader());
...
/*     */   {
/* 109 */     return (T)convertIfNecessary(null, null, newValue, requiredType, methodParam != null ? new TypeDescriptor(methodParam) : 
...
/*     */     throws IllegalArgumentException
/*     */   {
/* 126 */     return (T)convertIfNecessary(null, null, newValue, requiredType, field != null ? new TypeDescriptor(field) : 
...
/*     */     throws IllegalArgumentException
/*     */   {
/* 144 */     return (T)convertIfNecessary(propertyName, oldValue, newValue, requiredType, TypeDescriptor.valueOf(requiredType));
/*     */   }
/*     */   
/*     */ 
/*     */ 
...
/* 193 */             convertedValue = StringUtils.commaDelimitedListToStringArray((String)convertedValue);
/*     */           }
/*     */         }
/*     */       }
/* 197 */       if (editor == null) {
...
/*     */       }
/* 200 */       convertedValue = doConvertValue(oldValue, convertedValue, requiredType, editor);
/*     */     }
/*     */     
...
/* 215 */             convertedValue = StringUtils.commaDelimitedListToStringArray((String)convertedValue);
/*     */           }
...
/*     */         {
/* 221 */           convertedValue = convertToTypedCollection((Collection)convertedValue, propertyName, requiredType, typeDescriptor);
/*     */           
/* 223 */           standardConversion = true;
/*     */         }
/* 225 */         else if ((convertedValue instanceof Map))
/*     */         {
/* 227 */           convertedValue = convertToTypedMap((Map)convertedValue, propertyName, requiredType, typeDescriptor);
/*     */           
/* 229 */           standardConversion = true;
/*     */         }
/* 231 */         if ((convertedValue.getClass().isArray()) && (Array.getLength(convertedValue) == 1)) {
/* 232 */           convertedValue = Array.get(convertedValue, 0);
/* 233 */           standardConversion = true;
/*     */         }
/* 235 */         if ((String.class == requiredType) && (ClassUtils.isPrimitiveOrWrapper(convertedValue.getClass())))
/*     */         {
/* 237 */           return convertedValue.toString();
/*     */         }
...
/* 243 */               return (T)BeanUtils.instantiateClass(strCtor, new Object[] { convertedValue });
/*     */             }
...
/* 260 */             return null;
/*     */           }
/* 262 */           convertedValue = attemptToConvertStringToEnum(requiredType, trimmedValue, convertedValue);
/* 263 */           standardConversion = true;
/*     */         }
/* 265 */         else if (((convertedValue instanceof Number)) && (Number.class.isAssignableFrom(requiredType))) {
/* 266 */           convertedValue = NumberUtils.convertNumberToTargetClass((Number)convertedValue, requiredType);
/*     */           
...
/* 274 */         convertedValue = javaUtilOptionalEmpty;
/*     */       }
/*     */       
/*     */ 
...
/*     */         {
/*     */ 
/* 302 */           msg.append(": PropertyEditor [").append(editor.getClass().getName()).append("] returned inappropriate value of type '").append(ClassUtils.getDescriptiveType(convertedValue)).append("'");
...
/*     */       }
/* 316 */       logger.debug("Original ConversionService attempt failed - ignored since PropertyEditor based conversion eventually succeeded", conversionAttemptEx);
/*     */     }
/*     */     
/*     */ 
...
/*     */       }
/* 433 */       convertedValue = StringUtils.arrayToCommaDelimitedString((String[])convertedValue);
/*     */     }
/*     */     
...
/* 478 */       Object result = Array.newInstance(componentType, coll.size());
/* 479 */       int i = 0;
/* 480 */       for (Iterator<?> it = coll.iterator(); it.hasNext(); i++) {
/* 481 */         Object value = convertIfNecessary(
/* 482 */           buildIndexedPropertyName(propertyName, i), null, it.next(), componentType);
/* 483 */         Array.set(result, i, value);
/*     */       }
/* 485 */       return result;
/*     */     }
...
/* 494 */       Object result = Array.newInstance(componentType, arrayLength);
/* 495 */       for (int i = 0; i < arrayLength; i++) {
/* 496 */         Object value = convertIfNecessary(
/* 497 */           buildIndexedPropertyName(propertyName, i), null, Array.get(input, i), componentType);
/* 498 */         Array.set(result, i, value);
/*     */       }
/* 500 */       return result;
/*     */     }
/*     */     
...
/*     */     try
/*     */     {
/* 538 */       Iterator<?> it = original.iterator();
/* 539 */       if (it == null) {
...
/*     */       }
/*     */       else {
/* 561 */         convertedCopy = (Collection)requiredType.newInstance();
/*     */       }
/*     */     } catch (Throwable ex) {
/*     */       Collection<Object> convertedCopy;
...
/*     */     try
/*     */     {
/* 620 */       Iterator<?> it = original.entrySet().iterator();
/* 621 */       if (it == null) {
...
/*     */       }
/*     */       else {
/* 643 */         convertedCopy = (Map)requiredType.newInstance();
/*     */       }
/*     */     } catch (Throwable ex) {
/*     */       Map<Object, Object> convertedCopy;
...
/* Location:              C:\Users\ikatwal\Downloads\bullet-zone-server-2.0.jar!\BOOT-INF\lib\spring-beans-4.3.14.RELEASE.jar!\org\springframework\beans\TypeConverterDelegate.class
