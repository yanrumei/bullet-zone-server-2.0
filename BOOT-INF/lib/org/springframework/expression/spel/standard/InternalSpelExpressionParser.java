...
/*      */   
/*      */ 
/*   94 */   private final Stack<SpelNodeImpl> constructedNodes = new Stack();
/*      */   
/*      */ 
/*      */ 
...
/*  352 */       ((SpelNodeImpl)nodes.get(nodes.size() - 1)).getEndPosition()), 
/*  353 */       (SpelNodeImpl[])nodes.toArray(new SpelNodeImpl[nodes.size()]));
/*      */   }
/*      */   
...
/*  424 */     if (args == null) {
/*  425 */       push(new VariableReference(functionOrVariableName.data, 
/*  426 */         toPos(t.startPos, functionOrVariableName.endPos)));
/*  427 */       return true;
/*      */     }
/*      */     
/*  430 */     push(new FunctionReference(functionOrVariableName.data, 
...
/*  443 */     return (SpelNodeImpl[])args.toArray(new SpelNodeImpl[args.size()]);
/*      */   }
/*      */   
/*      */   private void eatConstructorArgs(List<SpelNodeImpl> accumulatedArguments) {
/*  447 */     if (!peekToken(TokenKind.LPAREN))
/*      */     {
/*  449 */       throw new InternalParseException(new SpelParseException(this.expressionString, positionOf(peekToken()), SpelMessage.MISSING_CONSTRUCTOR_ARGS, new Object[0]));
/*      */     }
...
/*  528 */       Token beanRefToken = nextToken();
...
/*      */       {
/*  571 */         push(new PropertyOrFieldReference(false, t.data, toPos(t)));
/*  572 */         return true;
/*      */       }
...
/*      */     {
/*  626 */       expr = new InlineList(toPos(t.startPos, closingCurly.endPos), new SpelNodeImpl[0]);
/*      */     }
/*  628 */     else if (peekToken(TokenKind.COLON, true)) {
/*  629 */       closingCurly = eatToken(TokenKind.RCURLY);
/*      */       
/*  631 */       expr = new InlineMap(toPos(t.startPos, closingCurly.endPos), new SpelNodeImpl[0]);
/*      */     }
/*      */     else {
...
/*      */         
/*  644 */         expr = new InlineList(toPos(t.startPos, closingCurly.endPos), (SpelNodeImpl[])listElements.toArray(new SpelNodeImpl[listElements.size()]));
/*      */       }
/*  646 */       else if (peekToken(TokenKind.COMMA, true)) {
/*  647 */         List<SpelNodeImpl> listElements = new ArrayList();
/*  648 */         listElements.add(firstExpression);
/*      */         do {
/*  650 */           listElements.add(eatExpression());
/*      */         }
/*  652 */         while (peekToken(TokenKind.COMMA, true));
/*  653 */         closingCurly = eatToken(TokenKind.RCURLY);
/*      */         
/*  655 */         expr = new InlineList(toPos(t.startPos, closingCurly.endPos), (SpelNodeImpl[])listElements.toArray(new SpelNodeImpl[listElements.size()]));
/*      */ 
/*      */       }
/*  658 */       else if (peekToken(TokenKind.COLON, true)) {
/*  659 */         List<SpelNodeImpl> mapElements = new ArrayList();
/*  660 */         mapElements.add(firstExpression);
/*  661 */         mapElements.add(eatExpression());
/*  662 */         while (peekToken(TokenKind.COMMA, true)) {
/*  663 */           mapElements.add(eatExpression());
/*  664 */           eatToken(TokenKind.COLON);
/*  665 */           mapElements.add(eatExpression());
/*      */         }
/*  667 */         closingCurly = eatToken(TokenKind.RCURLY);
/*      */         
/*  669 */         expr = new InlineMap(toPos(t.startPos, closingCurly.endPos), (SpelNodeImpl[])mapElements.toArray(new SpelNodeImpl[mapElements.size()]));
/*      */       }
/*      */       else {
/*  672 */         raiseInternalException(t.startPos, SpelMessage.OOD, new Object[0]);
/*      */       }
/*      */     }
/*  675 */     this.constructedNodes.push(expr);
/*  676 */     return true;
/*      */   }
/*      */   
...
/*      */   {
/*  718 */     LinkedList<SpelNodeImpl> qualifiedIdPieces = new LinkedList();
...
/*      */     }
/*  734 */     int pos = toPos(((SpelNodeImpl)qualifiedIdPieces.getFirst()).getStartPosition(), 
/*  735 */       ((SpelNodeImpl)qualifiedIdPieces.getLast()).getEndPosition());
/*  736 */     return new QualifiedIdentifier(pos, 
/*  737 */       (SpelNodeImpl[])qualifiedIdPieces.toArray(new SpelNodeImpl[qualifiedIdPieces.size()]));
/*      */   }
/*      */   
...
/*  758 */       if (args == null)
/*      */       {
/*  760 */         push(new PropertyOrFieldReference(nullSafeNavigation, methodOrPropertyName.data, 
/*  761 */           toPos(methodOrPropertyName)));
/*  762 */         return true;
/*      */       }
/*      */       
/*  765 */       push(new MethodReference(nullSafeNavigation, methodOrPropertyName.data, 
...
/*      */       {
/*  781 */         push(new PropertyOrFieldReference(false, newToken.data, toPos(newToken)));
/*  782 */         return true;
/*      */       }
/*  784 */       SpelNodeImpl possiblyQualifiedConstructorName = eatPossiblyQualifiedId();
/*  785 */       List<SpelNodeImpl> nodes = new ArrayList();
/*  786 */       nodes.add(possiblyQualifiedConstructorName);
/*  787 */       if (peekToken(TokenKind.LSQUARE))
/*      */       {
/*  789 */         List<SpelNodeImpl> dimensions = new ArrayList();
...
/*  800 */           nodes.add(pop());
/*      */         }
/*  802 */         push(new ConstructorReference(toPos(newToken), 
/*  803 */           (SpelNodeImpl[])dimensions.toArray(new SpelNodeImpl[dimensions.size()]), 
/*  804 */           (SpelNodeImpl[])nodes.toArray(new SpelNodeImpl[nodes.size()])));
/*      */       }
/*      */       else
/*      */       {
/*  808 */         eatConstructorArgs(nodes);
/*      */         
/*  810 */         push(new ConstructorReference(toPos(newToken), 
/*  811 */           (SpelNodeImpl[])nodes.toArray(new SpelNodeImpl[nodes.size()])));
/*      */       }
/*  813 */       return true;
/*      */     }
/*  815 */     return false;
...
/*  823 */     return (SpelNodeImpl)this.constructedNodes.pop();
/*      */   }
/*      */   
/*      */ 
/*      */ 
...
/*  840 */       push(Literal.getIntLiteral(t.data, toPos(t), 10));
/*      */     }
/*  842 */     else if (t.kind == TokenKind.LITERAL_LONG) {
/*  843 */       push(Literal.getLongLiteral(t.data, toPos(t), 10));
/*      */     }
/*  845 */     else if (t.kind == TokenKind.LITERAL_HEXINT) {
/*  846 */       push(Literal.getIntLiteral(t.data, toPos(t), 16));
/*      */     }
/*  848 */     else if (t.kind == TokenKind.LITERAL_HEXLONG) {
/*  849 */       push(Literal.getLongLiteral(t.data, toPos(t), 16));
/*      */     }
/*  851 */     else if (t.kind == TokenKind.LITERAL_REAL) {
/*  852 */       push(Literal.getRealLiteral(t.data, toPos(t), false));
/*      */     }
/*  854 */     else if (t.kind == TokenKind.LITERAL_REAL_FLOAT) {
/*  855 */       push(Literal.getRealLiteral(t.data, toPos(t), true));
/*      */     }
/*  857 */     else if (peekIdentifierToken("true")) {
/*  858 */       push(new BooleanLiteral(t.data, toPos(t), true));
/*      */     }
/*  860 */     else if (peekIdentifierToken("false")) {
/*  861 */       push(new BooleanLiteral(t.data, toPos(t), false));
/*      */     }
/*  863 */     else if (t.kind == TokenKind.LITERAL_STRING) {
/*  864 */       push(new StringLiteral(t.data, toPos(t), t.data));
/*      */     }
/*      */     else {
/*  867 */       return false;
/*      */     }
...
/*  992 */       return null;
/*      */     }
/*  994 */     return (Token)this.tokenStream.get(this.tokenStreamPointer++);
/*      */   }
/*      */   
...
/*  999 */       return null;
/*      */     }
/* 1001 */     return (Token)this.tokenStream.get(this.tokenStreamPointer);
/*      */   }
/*      */   
...
/* Location:              C:\Users\ikatwal\Downloads\bullet-zone-server-2.0.jar!\BOOT-INF\lib\spring-expression-4.3.14.RELEASE.jar!\org\springframework\expression\spel\standard\InternalSpelExpressionParser.class
