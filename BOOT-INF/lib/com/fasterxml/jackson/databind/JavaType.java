/*     */ package com.fasterxml.jackson.databind;
/*     */ 
/*     */ import com.fasterxml.jackson.core.type.ResolvedType;
/*     */ import com.fasterxml.jackson.databind.type.TypeBindings;
/*     */ import com.fasterxml.jackson.databind.type.TypeFactory;
/*     */ import java.io.Serializable;
/*     */ import java.lang.reflect.Modifier;
/*     */ import java.lang.reflect.Type;
/*     */ import java.util.List;
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ public abstract class JavaType
/*     */   extends ResolvedType
/*     */   implements Serializable, Type
/*     */ {
/*     */   private static final long serialVersionUID = 1L;
/*     */   protected final Class<?> _class;
/*     */   protected final int _hash;
/*     */   protected final Object _valueHandler;
/*     */   protected final Object _typeHandler;
/*     */   protected final boolean _asStatic;
/*     */   
/*     */   protected JavaType(Class<?> raw, int additionalHash, Object valueHandler, Object typeHandler, boolean asStatic)
/*     */   {
/*  79 */     this._class = raw;
/*  80 */     this._hash = (raw.getName().hashCode() + additionalHash);
/*  81 */     this._valueHandler = valueHandler;
/*  82 */     this._typeHandler = typeHandler;
/*  83 */     this._asStatic = asStatic;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   protected JavaType(JavaType base)
/*     */   {
/*  93 */     this._class = base._class;
/*  94 */     this._hash = base._hash;
/*  95 */     this._valueHandler = base._valueHandler;
/*  96 */     this._typeHandler = base._typeHandler;
/*  97 */     this._asStatic = base._asStatic;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract JavaType withTypeHandler(Object paramObject);
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract JavaType withContentTypeHandler(Object paramObject);
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract JavaType withValueHandler(Object paramObject);
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract JavaType withContentValueHandler(Object paramObject);
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public JavaType withHandlersFrom(JavaType src)
/*     */   {
/* 141 */     JavaType type = this;
/* 142 */     Object h = src.getTypeHandler();
/* 143 */     if (h != this._typeHandler) {
/* 144 */       type = type.withTypeHandler(h);
/*     */     }
/* 146 */     h = src.getValueHandler();
/* 147 */     if (h != this._valueHandler) {
/* 148 */       type = type.withValueHandler(h);
/*     */     }
/* 150 */     return type;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract JavaType withContentType(JavaType paramJavaType);
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract JavaType withStaticTyping();
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract JavaType refine(Class<?> paramClass, TypeBindings paramTypeBindings, JavaType paramJavaType, JavaType[] paramArrayOfJavaType);
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   @Deprecated
/*     */   public JavaType forcedNarrowBy(Class<?> subclass)
/*     */   {
/* 211 */     if (subclass == this._class) {
/* 212 */       return this;
/*     */     }
/* 214 */     JavaType result = _narrow(subclass);
/*     */     
/* 216 */     if (this._valueHandler != result.getValueHandler()) {
/* 217 */       result = result.withValueHandler(this._valueHandler);
/*     */     }
/* 219 */     if (this._typeHandler != result.getTypeHandler()) {
/* 220 */       result = result.withTypeHandler(this._typeHandler);
/*     */     }
/* 222 */     return result;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */   @Deprecated
/*     */   protected abstract JavaType _narrow(Class<?> paramClass);
/*     */   
/*     */ 
/*     */ 
/*     */   public final Class<?> getRawClass()
/*     */   {
/* 235 */     return this._class;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */   public final boolean hasRawClass(Class<?> clz)
/*     */   {
/* 243 */     return this._class == clz;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public boolean hasContentType()
/*     */   {
/* 253 */     return true;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */   public final boolean isTypeOrSubTypeOf(Class<?> clz)
/*     */   {
/* 260 */     return (this._class == clz) || (clz.isAssignableFrom(this._class));
/*     */   }
/*     */   
/*     */   public boolean isAbstract()
/*     */   {
/* 265 */     return Modifier.isAbstract(this._class.getModifiers());
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public boolean isConcrete()
/*     */   {
/* 275 */     int mod = this._class.getModifiers();
/* 276 */     if ((mod & 0x600) == 0) {
/* 277 */       return true;
/*     */     }
/*     */     
/*     */ 
/*     */ 
/* 282 */     return this._class.isPrimitive();
/*     */   }
/*     */   
/*     */   public boolean isThrowable() {
/* 286 */     return Throwable.class.isAssignableFrom(this._class);
/*     */   }
/*     */   
/* 289 */   public boolean isArrayType() { return false; }
/*     */   
/*     */   public final boolean isEnumType() {
/* 292 */     return this._class.isEnum();
/*     */   }
/*     */   
/* 295 */   public final boolean isInterface() { return this._class.isInterface(); }
/*     */   
/*     */   public final boolean isPrimitive() {
/* 298 */     return this._class.isPrimitive();
/*     */   }
/*     */   
/* 301 */   public final boolean isFinal() { return Modifier.isFinal(this._class.getModifiers()); }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract boolean isContainerType();
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public boolean isCollectionLikeType()
/*     */   {
/* 316 */     return false;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */   public boolean isMapLikeType()
/*     */   {
/* 324 */     return false;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public final boolean isJavaLangObject()
/*     */   {
/* 335 */     return this._class == Object.class;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public final boolean useStaticType()
/*     */   {
/* 345 */     return this._asStatic;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public boolean hasGenericTypes()
/*     */   {
/* 354 */     return containedTypeCount() > 0;
/*     */   }
/*     */   
/* 357 */   public JavaType getKeyType() { return null; }
/*     */   
/*     */   public JavaType getContentType() {
/* 360 */     return null;
/*     */   }
/*     */   
/* 363 */   public JavaType getReferencedType() { return null; }
/*     */   
/*     */ 
/*     */   public abstract int containedTypeCount();
/*     */   
/*     */ 
/*     */   public abstract JavaType containedType(int paramInt);
/*     */   
/*     */ 
/*     */   @Deprecated
/*     */   public abstract String containedTypeName(int paramInt);
/*     */   
/*     */   @Deprecated
/*     */   public Class<?> getParameterSource()
/*     */   {
/* 378 */     return null;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public JavaType containedTypeOrUnknown(int index)
/*     */   {
/* 404 */     JavaType t = containedType(index);
/* 405 */     return t == null ? TypeFactory.unknownType() : t;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract TypeBindings getBindings();
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract JavaType findSuperType(Class<?> paramClass);
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract JavaType getSuperClass();
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract List<JavaType> getInterfaces();
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract JavaType[] findTypeParameters(Class<?> paramClass);
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public <T> T getValueHandler()
/*     */   {
/* 458 */     return (T)this._valueHandler;
/*     */   }
/*     */   
/*     */ 
/*     */   public <T> T getTypeHandler()
/*     */   {
/* 464 */     return (T)this._typeHandler;
/*     */   }
/*     */   
/*     */   public Object getContentValueHandler()
/*     */   {
/* 469 */     return null;
/*     */   }
/*     */   
/*     */   public Object getContentTypeHandler()
/*     */   {
/* 474 */     return null;
/*     */   }
/*     */   
/*     */   public boolean hasValueHandler()
/*     */   {
/* 479 */     return this._valueHandler != null;
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public boolean hasHandlers()
/*     */   {
/* 490 */     return (this._typeHandler != null) || (this._valueHandler != null);
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public String getGenericSignature()
/*     */   {
/* 510 */     StringBuilder sb = new StringBuilder(40);
/* 511 */     getGenericSignature(sb);
/* 512 */     return sb.toString();
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract StringBuilder getGenericSignature(StringBuilder paramStringBuilder);
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public String getErasedSignature()
/*     */   {
/* 531 */     StringBuilder sb = new StringBuilder(40);
/* 532 */     getErasedSignature(sb);
/* 533 */     return sb.toString();
/*     */   }
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract StringBuilder getErasedSignature(StringBuilder paramStringBuilder);
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract String toString();
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public abstract boolean equals(Object paramObject);
/*     */   
/*     */ 
/*     */ 
/*     */ 
/*     */ 
/*     */   public final int hashCode()
/*     */   {
/* 562 */     return this._hash;
/*     */   }
/*     */ }


/* Location:              C:\Users\ikatwal\Downloads\bullet-zone-server-2.0.jar!\BOOT-INF\lib\jackson-databind-2.8.10.jar!\com\fasterxml\jackson\databind\JavaType.class
 * Java compiler version: 7 (51.0)
 * JD-Core Version:       0.7.1
 */